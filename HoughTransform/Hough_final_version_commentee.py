...
pasXY = 1                       # Pas des points gardés pour la transformé (A laisser à 1, utiliser plutôt le resize)

# Transformée de Hough pour une droite
...
donnees ='' # Donnees transmises

def envoyer(a,b,r,width,heigth):
    a = int(1000*(a/width))/1000                     #On ne récupère que 3 chiffres significatifs pour les coordonnées et le centre
...
def point(arr,radi,x,y,Cmax):                       # Tranformation d'un point, arr = Accumulation; radi = Nombre d'angle; x,y = Coordonées
    for r in range(Rmin,Rmax,pasR):                 # On parcourt les différents rayons
        for i in range(radi):
            teta = 2*pi*i/radi
...
def hough_transform(im):
    ...
    Cmax = [0,0,0,0] # On récupère a,b,r,valeur
    tot_array = np.zeros((height+2*Rmax,width+2*Rmax,Rmax))
    k = 0
    for i in range(0,height,pasXY):
        for j in range(0,width,pasXY):
            if im[i][j]!=0:
                k +=1
...
def affichage(Cmax,original_image, blured, im):
    ...
    ax[0].set_title('Image originale', fontsize=20)
...
# fonction Resize pour réduire la résolution (travail par zone) qui prendra en argument le seuil, l'"image et le pas
...
def hough_droite(img,pasTheta, nbrMax, original_image):
    ...
    while (len(ldroite) != 2):
        ...
        if (len(ldroite)==0):
            ldroite.append([a,b])                                               # On ajoute le dépassement
        else:
...
while not(donnees=='end\r'):
    ...
    camera.resolution = (1024,768)                          #On définit les dimensions de la caméra que l'on utilisera

    while not(donnees=='stop\r' or donnees =='end\r'):

        start_time = time.time()                            #On lance un timer pour contrôler le temps d'une transformée

        print("Début")

        camera.start_preview()                              #Lance la caméra
        camera.capture(link)                                #on fait une capture de l'image que l'on a pris

        camera.stop_preview()                               #Arrête l'affichage de la caméra

        original_image = mpimg.imread(link)                 #on lit l'image stockée sur le link
        R,G,B = original_image[:,:,0],original_image[:,:,1],original_image[:,:,2] # Passage en noir et blanc
        ima = 0.2989*R + 0.5870*G + 0.1140*B                #On convertit l'image en noir et blanc selon la norme UIT BT.601-7,
        blured = gaussian_filter(ima, sigma=1)              # On applique un filtre gaussien (passe bas)
        ...
        xim, yim = im.shape
        im = im[2:xim-2,2:yim-2]                            #On évite les artefacts sur les bords

        print("Début hough cercle")
        Cmax,k = hough_transform(im)                        # On applique la transformée de hough circulaire
        print("Ok hough cercle")

        print("Début hough droite")
        l_droite = hough_droite(im,pasTheta,nbrDroite,original_image) # On applique la transformée de hough sur les droites
        print("Ok hough droite")

        #affichage(Cmax,original_image, blured, im)

        coin = droite(l_droite,im) # On définit le coin de l'image
        print("Ok coin")

        C = [Cmax[0]-Rmax,Cmax[1]-Rmax]                 # On supprime Rmax avant : On revient sur les dimensions de l'image initialement
        ...
        height,width = im.shape                         # On reprend les tailles originales
        donnees = envoyer(a,b,r,width,height)           # On envoie les données au téléphone
...
serveur.close()                                         # On ferme le Serveur
